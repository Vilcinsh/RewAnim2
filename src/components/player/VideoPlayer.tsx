...
export default function VideoPlayer({ streamUrl, intro, outro, onNextEpisode, hasNextEpisode, onProgress, initialSettings, onSettingsChange }: Props) {
  const videoRef = useRef<HTMLVideoElement>(null);
  const containerRef = useRef<HTMLDivElement>(null);
  const hlsRef = useRef<Hls | null>(null);
  const hideTimer = useRef<ReturnType<typeof setTimeout>>(undefined);
  const nextEpTimer = useRef<ReturnType<typeof setInterval>>(undefined);
  const settingsRef = useRef<Settings>({ autoSkipIntro: true, autoSkipOutro: true, autoNextEp: true, speed: 1, ...initialSettings });
  ...
  const [settings, setSettings] = useState<Settings>({ autoSkipIntro: true, autoSkipOutro: true, autoNextEp: true, speed: 1, ...initialSettings });
  ...
  useEffect(() => {
    ...
    if (Hls.isSupported()) {
      ...
      hls.on(Hls.Events.ERROR, (_e, data) => {
        ...
        else { setHlsError(true); hls.destroy(); }
      });
  ...
  // Progress reporting interval (every 15s while playing)
  useEffect(() => {
  ...
  // Drag seek: global mouse handlers
  useEffect(() => {
    ...
    function onMouseUp(e: MouseEvent) {
      ...
      setDragPct(null);
      const v = videoRef.current;
      if (v && durationRef.current > 0) v.currentTime = (pct / 100) * durationRef.current;
    }
  ...
  function onBarMouseMove(e: React.MouseEvent<HTMLDivElement>) {
    if (isDragging.current) return; // handled globally
    const bar = progressBarRef.current;
    if (!bar) return;
    const rect = bar.getBoundingClientRect();
    ...
    nextEpTimer.current = setInterval(() => {
      setNextEpCount(prev => {
        if (prev === null || prev <= 1) {
          ...
          if (settingsRef.current.autoNextEp) onNextEpisode?.();
          return null;
        }
        return prev - 1;
      });
  ...
  function onTimeUpdate() {
    ...
    if (!isDragging.current) setCurrentTime(t);
    if (v.buffered.length > 0) setBuffered(v.buffered.end(v.buffered.length - 1));

    const s = settingsRef.current;
    const i = introRef.current;
    const o = outroRef.current;
    if (s.autoSkipIntro && i && t >= i.start && t < i.end) v.currentTime = i.end;
    if (s.autoSkipOutro && o && t >= o.start && t < o.end) v.currentTime = o.end;

    // Intercept native subtitle rendering: flip any 'showing' track to 'hidden'
    // so the browser doesn't draw the default black-box caption, then read activeCues ourselves.
    let cueText: string | null = null;
    for (const track of Array.from(v.textTracks)) {
      if (track.kind !== 'subtitles' && track.kind !== 'captions') continue;
      if (track.mode === 'showing') track.mode = 'hidden';
      if (track.mode === 'hidden' && track.activeCues?.length) {
        cueText = (track.activeCues[0] as VTTCue).text.replace(/<[^>]*>/g, '').trim() || null;
        break;
      }
    }
  ...
  return (
    <div
      ...
      onMouseMove={() => { showCtrl(); if (playing) scheduleHide(); }}
      onMouseLeave={() => { playing && setShowControls(false); setHoverPct(null); }}
    >
      <video
        ref={videoRef}
        ...
        onPause={() => {
          ...
          showCtrl();
          const v = videoRef.current;
          if (v && v.duration) onProgressRef.current?.(v.currentTime, v.duration);
        }}
        ...
        onEnded={() => {
          ...
          showCtrl();
          const v = videoRef.current;
          if (v && v.duration) onProgressRef.current?.(v.duration, v.duration);
          if (hasNextEpisode) startNextEpCountdown();
        }}
      />
      ...
      {activeCueText && (
        <div className={`absolute left-0 right-0 flex justify-center pointer-events-none px-10 z-10 transition-all duration-150 ${ctrlVisible ? 'bottom-20' : 'bottom-4'}`}>
          <p className="text-white text-center text-[15px] font-medium leading-snug"
            style={{ textShadow: '0 1px 4px #000, 0 0 12px rgba(0,0,0,0.9)', whiteSpace: 'pre-line' }}>
            {activeCueText}
          </p>
        </div>
      )}
      ...
      {hlsError && (
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 pointer-events-none">
          <svg className="w-10 h-10 text-[var(--primary)]/50" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z"/></svg>
      ...
      {inIntro && !settings.autoSkipIntro && (
        <button
          onClick={() => { if (videoRef.current && introRef.current) videoRef.current.currentTime = introRef.current.end; }}
      ...
      {inOutro && !settings.autoSkipOutro && (
        <button
          onClick={() => { if (videoRef.current && outroRef.current) videoRef.current.currentTime = outroRef.current.end; }}
          className="absolute top-4 right-4 z-20 flex items-center gap-2 bg-black/75 border border-white/20 backdrop-blur-sm text-white text-sm font-medium px-4 py-2 rounded-lg hover:bg-[var(--primary)] hover:border-[var(--primary)] transition-all"
        >
          Izlaist outro
          <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M6 18l8.5-6L6 6v12zm2-8.14L11.03 12 8 14.14V9.86zM16 6h2v12h-2z"/></svg>
      ...
      {nextEpCount !== null && hasNextEpisode && (
        <div className="absolute bottom-24 right-4 z-20 bg-black/85 border border-white/15 backdrop-blur-sm rounded-xl p-4 text-white min-w-48">
          ...
          <div className="flex gap-2">
            <button onClick={() => { cancelNextEp(); onNextEpisode?.(); }}
      ...
      {/* Controls */}
      <div className={`absolute inset-0 flex flex-col justify-end transition-opacity duration-300 ${ctrlVisible ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
        <div className="bg-gradient-to-t from-black/90 via-black/40 to-transparent px-4 pb-4 pt-16">
          ...
          >
            ...
            >
              ...
              {/* Intro marker */}
              {intro && duration > 0 && (
                <div className="absolute inset-y-0 bg-yellow-400/50 pointer-events-none rounded-full"
                  style={{ left: `${(intro.start / duration) * 100}%`, width: `${((intro.end - intro.start) / duration) * 100}%` }} />
              )}
              {/* Outro marker */}
              {outro && duration > 0 && (
                <div className="absolute inset-y-0 bg-sky-400/50 pointer-events-none rounded-full"
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-white/50 text-xs tabular-nums shrink-0">{fmt(currentTime)} / {fmt(duration)}</span>
            <div className="flex-1" />

            <select value={settings.speed} onChange={e => setSettings(s => ({ ...s, speed: Number(e.target.value) }))}
              ...
              {SPEEDS.map(s => <option key={s} value={s} className="bg-[#1a1a1a]">{s}x</option>)}
            </select>
            ...
            <div className="relative shrink-0">
              ...
              {showSettings && (
                <div className="absolute bottom-9 right-0 bg-[var(--surface)] border border-[var(--border)] rounded-xl p-4 w-60 shadow-2xl z-30"
                  ...
                  <div className="pt-2">
                    ...
                    <div className="flex gap-1 flex-wrap">
                      {SPEEDS.map(s => (
                        <button key={s} onClick={() => setSettings(st => ({ ...st, speed: s }))}
